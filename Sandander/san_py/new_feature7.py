...
def new_cust(row):
    dict = {0:0, 1:1, 10:2, 11:3, 12:4, 20:5, 21:6, 30:7, 31:8}
    sex = row['ind_nuevo'].strip()
    if sex in ['','NA']:
        sex = mapping_dict["ind_nuevo"][-99]
    else:
        sex = mapping_dict["ind_nuevo"][sex]

    acticity = row["segmento"].strip()
    if acticity in ['','NA']:
        acticity = mapping_dict["segmento"][-99]*10
    else:
        acticity = mapping_dict["segmento"][acticity]*10
    new_index = sex+acticity
...
def processDataMK(in_file_name, cust_dict, lag_cust_dict,total_dict,lag1,lag2,lag3,lag12,lag_feature):
    ...
    for row in csv.DictReader(in_file_name):
        ...
        if (row['fecha_dato'] in ['2015-01-28', '2016-01-28'] ):
            ...
            lag_feature[cust_id].append(income_cate(row))
            lag_feature[cust_id].append(getAge_group(row))
            lag_feature[cust_id].append(new_cust(row))
            continue
...
if __name__ == "__main__":
    ...
    # x_vars_list = np.load('train_xnf7.npy').tolist()
    # y_vars_list = np.load('train_ynf7.npy').tolist()
    ...
    np.save('test_xnf7.npy', x_vars_list)
    ...
    # plot_importance(xgb.train(plst, xgtrain, num_rounds))
    # pyplot.show()

    # plst = list(param.items())
    # tuned_parameters = [{'min_child_weight': [10, 11, 12,13]}]
    # cv = GridSearchCV(XGBClassifier(objective= 'multi:softprob', learning_rate = 0.09, max_depth=5,
    #                                 subsample=0.85, colsample_bytree=0.9,), tuned_parameters, cv=5)
