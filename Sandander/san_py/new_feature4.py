...
def getCustSeniority_group(row):
    min_value = 0.
    max_value = 256.
    range_value = max_value - min_value
    missing_value = 0.
    cust_seniority = row['antiguedad'].strip()
    if cust_seniority == 'NA' or cust_seniority == '' or cust_seniority == -999999:
        cust_seniority = missing_value
    else:
        cust_seniority = float(cust_seniority)
        if cust_seniority < min_value:
    ...
    if 0 <= cust_seniority <= 50:
        return 0
    if 51 <= cust_seniority <= 100:
        return 1
    if 101 <= cust_seniority <= 150:
        return 2
    if 151 <= cust_seniority <= 200:
        return 3
    if 202 <= cust_seniority:
        return 4
    return 5
...
#sex and activity index
def sex_activity(row):
    dict = {"H 1":0, "H 0":1, "H N":2, "V 1":3 ,"V 0":4, "V N":5, "N 1":6, "N 0":7, "NN":8,
            "H1":0, "H0":1, "HN":2, "V1":3,"V0":4, "VN":5, "N1":6, "N0":7}
    sex = row['sexo']
    if sex in ['','NA']:
        sex = -99
    if sex == -99:
        sex = "N"
    acticity = row["ind_actividad_cliente"]
    if acticity in ['','NA']:
        acticity = -99
    if acticity == -99:
        acticity = "N"
    new_index = sex+acticity
    return dict[new_index]

def processDataMK(in_file_name, cust_dict, lag_cust_dict,total_dict,lag_feature,lag1,lag_feature1):
    x_vars_list = []
    y_vars_list = []

    for row in csv.DictReader(in_file_name):
        if row['fecha_dato'] not in ['2015-01-28', '2015-04-28','2015-05-28', '2015-06-28', '2016-01-28', '2016-04-28','2016-05-28', '2016-06-28']:
            continue
        ...
        if (row['fecha_dato'] in ['2015-01-28', '2016-01-28'] ):
            ...
            lag_feature[cust_id] = [getAge(row)]
            # sex = getIndex(row, 'sexo')
            # age = getAge(row)
            #
            # lag_feature[cust_id].append( getMonth(row))
            # lag_feature[cust_id].append(getCustSeniority(row))
            # lag_feature[cust_id].append(getMarriageIndex(row, age, sex, income) )
            #
            #
            # lag_feature[cust_id].append(sex_activity(row))
            lag_feature[cust_id].append(getTotalTarget(row))
            lag_feature[cust_id].append(getCustSeniority_group(row))
            income = getRent(row)
            lag_feature[cust_id].append(income)
            lag_feature[cust_id].append( getjoinMonth(row))
            continue

        if (row['fecha_dato'] in ['2015-04-28', '2016-04-28'] ):
            target_list = getTarget(row)
            lag1[cust_id]= target_list[:]
            lag_feature1[cust_id] = [getAge(row)]
            # sex = getIndex(row, 'sexo')
            # age = getAge(row)
            #
            # lag_feature[cust_id].append( getMonth(row))
            ...
            #
            # lag_feature[cust_id].append(sex_activity(row))
            lag_feature1[cust_id].append(getTotalTarget(row))
            lag_feature1[cust_id].append(getCustSeniority_group(row))
            income = getRent(row)
            lag_feature1[cust_id].append(income)
            lag_feature1[cust_id].append( getjoinMonth(row))

        if (row['fecha_dato'] in ['2015-05-28', '2016-05-28'] ):
            target_list = getTarget(row)
            cust_dict[cust_id] =  target_list[:]
            total_dict[cust_id] = [getTotalTarget(row)]
            continue
        ...
        # x_vars.append(getAge_group(row))
        x_vars.append(getCustSeniority_group(row))
        x_vars.append(sex_activity(row))
        # x_vars.append(income_cate(row))
        if row['fecha_dato'] == '2016-06-28':
            ...
            lag_feature_list = lag_feature.get(cust_id, [0.3162,0,5,0.0679,13])
            lag_feature_list1 = lag_feature1.get(cust_id, [0.3162,0,5,0.0679,13])
            x_vars_list.append(x_vars + prev_target_list + lag_target_list+total_list+lag_feature_list+lag_target_list1
                               +lag_feature_list1)
    ...
    return x_vars_list, y_vars_list, cust_dict, lag_cust_dict,total_dict,lag_feature,lag1,lag_feature1
...
if __name__ == "__main__":
	...
	data_path = "/Users/xiaofeifei/I/Kaggle/Santander/"
	train_file =  open(data_path + "train_ver2.csv")
	print('Starting file processing')
	#x_vars_list, y_vars_list, cust_dict = processData(train_file, {})
	x_vars_list, y_vars_list, cust_dict, lag_cust_dict, total_dict,lag_feature,lag_target_list1,lag_feature1 = \
	...
	x_vars_list, y_vars_list, cust_dict, lag_cust_dict, total_dict,lag_feature,lag_target_list1,lag_feature1 = \
        processDataMK(test_file, cust_dict, lag_cust_dict,total_dict,lag_feature,lag_target_list1,lag_feature1)
	...
	plst = list(param.items())
	xgtrain = xgb.DMatrix(train_X, label=train_y)
	print xgb.cv(plst, xgtrain, num_rounds)
