...
if __name__ == '__main__':
    data = pd.read_pickle('/Users/xiaofeifei/I/Kaggle/Basket/prediction_lgbm_my_data1.pkl')

    # data = pd.read_csv("/Users/xiaofeifei/I/Kaggle/Basket/ensemble/ensemble6.csv")
    df = data.loc[data.prediction > 0.05, ['order_id', 'prediction', 'product_id']]

    # Group products per order here
    df_order = applyParallel(df.groupby(df.order_id), create_products_faron).reset_index()

    df_order[['order_id', 'products']].to_csv('/Users/xiaofeifei/I/Kaggle/Basket/F1_Max_my_data1_new.csv', index=False)