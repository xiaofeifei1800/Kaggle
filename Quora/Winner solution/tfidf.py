...
if __name__ == "__main__":
    ...
    train  = pd.read_csv(input_folder + 'train_clean.csv', nrows = 1000)
    test = pd.read_csv(input_folder + 'test_clean.csv', nrows = 1000)
    ...
    for feat_name,column_name in zip(feat_names, column_names):
        ...
        svd = TruncatedSVD(n_components=svd_n_components, n_iter=15)
        X_svd_train = svd.fit_transform(X_tfidf_train)
        dfTrain[feat_name] = X_svd_train[:,1]
    ...
    dfTrain.to_csv(input_folder+"df_cooccur_git.csv", index=False)
    print("Done.")
