...
train_df = pd.read_csv(data_path+'train_all.csv', na_values="-1") # .iloc[0:200,:]
test_df = pd.read_csv(data_path+'test_all.csv', na_values="-1")

train_stack = pd.read_csv(data_path + 'train_stack.csv')
test_stack = pd.read_csv(data_path + 'test_stack.csv')

### merge
train_df = train_df.merge(train_stack, on = 'id', how = 'left')
test_df = test_df.merge(test_stack, on = 'id', how = 'left')
...
val.to_csv(data_out+'rgf_train_s1.csv', float_format='%.6f', index=False)

# Create submission file
sub = pd.DataFrame()
sub['id'] = id_test
sub['target'] = y_test_pred
sub.to_csv(data_out+'rgf_test_s1.csv', float_format='%.6f', index=False)