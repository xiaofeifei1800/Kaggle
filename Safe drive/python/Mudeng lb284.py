...
print(check_output(["ls", "/Users/xiaofeifei/I/Kaggle/Safe drive"]).decode("utf8"))
...
def read_data():
    root_dir = '/Users/xiaofeifei/I/Kaggle/Safe drive/'
    df_train = pd.read_csv(os.path.join(root_dir, 'train_all.csv'), na_values=-1)
    df_train.drop([149161], axis=0, inplace=True)
    df_test = pd.read_csv(os.path.join(root_dir, 'test_all.csv'), na_values=-1)

    train_stack = pd.read_csv(root_dir + 'train_stack.csv')
    test_stack = pd.read_csv(root_dir + 'test_stack.csv')

    ### merge
    df_train = df_train.merge(train_stack, on = 'id', how = 'left')
    df_test = df_test.merge(test_stack, on = 'id', how = 'left')
...
write_data(
    ...
    sub_filename='mudeng_test_s1.csv',
    train_filename='mudeng_train_s1.csv'
)
